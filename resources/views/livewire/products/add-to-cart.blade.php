                    ...
                    @foreach ($option->pivot->features as $feature)
                        <li>
                            ...
                            @switch($option->type)
                                @case(1)
                                    <button
                                        class="w-20 h-8 font-semibold uppercase text-sm rounded-lg {{ ($selectedFeatures[$option->id] ?? null) == $feature['id'] ? 'bg-red-500 text-white' : 'border border-gray-200 text-gray-700' }} "
                                ...
                                @case(2)
                                    <div
                                        class="p-0.5 border-2 rounded-lg flex items-center -mt-1.5 {{ ($selectedFeatures[$option->id] ?? null) == $feature['id'] ? 'border-green-600' : 'border-transparent' }}">
